...
# Blogger 配置（Google Blogger API v3）
BLOGGER_BLOG_ID = os.environ.get("BLOGGER_BLOG_ID", "6022996640395116239")
...
def get_google_access_token():
    """用 Refresh Token 获取 Google Access Token"""
...
def markdown_to_html(md_text):
    """将 Markdown 简单转换为 HTML（Blogger 需要 HTML 格式）"""
    html = md_text
...
def publish_to_blogger(title, content):
    ...
    if not access_token:
        print("    Blogger 发布失败：无法获取 Access Token")
        return False

    # Blogger API 接受 HTML 格式
    ...
    payload = {
        ...
        "labels": ["finance", "Philippines", "personal finance", "loans"],
    }

    resp = requests.post(BLOGGER_API_URL, json=payload, headers=headers, timeout=30)

    if resp.status_code in (200, 201):
        data = resp.json()
        print(f"    Blogger 发布成功: {data.get('url', 'N/A')}")
        return True
    else:
        print(f"    Blogger 发布失败 ({resp.status_code}): {resp.text[:200]}")
        return False
...
def get_blogger_refresh_token():
    ...
    print("=" * 60)
    print("Blogger OAuth2 授权工具")
    print("=" * 60)

    client_id = input("请输入 Google Client ID: ").strip()
    client_secret = input("请输入 Google Client Secret: ").strip()
    ...
    auth_code = input("授权后，请粘贴授权码: ").strip()
    ...
    if resp.status_code == 200:
        ...
        print(f"\n{'=' * 60}")
        print(f"授权成功！请保存以下信息到 GitHub Secrets：")
        print(f"{'=' * 60}")
        ...
        print(f"\n授权失败: {resp.status_code} {resp.text}")


# ============ 主流程 ============
def main():
    # 检查是否是获取 Blogger Token 模式
    if len(sys.argv) > 1 and sys.argv[1] == "--get-blogger-token":
